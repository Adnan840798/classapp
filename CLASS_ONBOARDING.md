...
To enable Telegram mirroring, allowing announcements and files posted on ClassApp to automatically broadcast to a Telegram channel:

### 9a. Create the Telegram Channel
1. Open Telegram, tap the new message icon (the pencil), and select **New Channel**.
2. Give it a name and description, and click **Create**.
3. Choose whether it is **Public** or **Private**:
   - **Public**: Anyone can search for and join it. You will create a custom username link (e.g. `t.me/myclasschannel`). In this case, your `TELEGRAM_CHANNEL_ID` env variable will simply be `@myclasschannel`.
   - **Private** *(Recommended for classrooms)*: Users can only join using an invite link.

### 9b. Add your Telegram Bot as an Admin
1. Open your channel settings (tap the channel header at the top).
2. Go to **Administrators** (or **Manage Channel > Administrators**).
3. Tap **Add Administrator**.
4. Search for your Bot's username (the bot you created using `@BotFather`).
5. Select the Bot, ensure the **Post Messages** permission is toggled **ON**, and save.

### 9c. Retrieve the Numeric Channel ID
If your channel is **Private**, you must retrieve its unique numeric ID:
1. Post a temporary test message in your new private channel.
2. Forward that message to the bot **`@ShowJsonBot`** (or **`@RawDataBot`**) on Telegram.
3. The bot will instantly reply with a JSON message. Look for the `"forward_from_chat"` or `"chat"` block and copy the `"id"` value (it will start with `-100`, for example `-1002148202934`).

### 9d. Connect it to the Application Environment
Update the environment variables so the server action can communicate with the channel:

#### For Production (Vercel)
1. Go to your **Vercel Dashboard** and open your project.
2. Navigate to **Settings > Environment Variables**.
3. Add or update these variables:
   - **`TELEGRAM_BOT_TOKEN`**: The API token generated by `@BotFather` when you created the bot.
   - **`TELEGRAM_CHANNEL_ID`**: The channel ID you obtained in Step 9c (e.g., `-100123456789` or `@myclasschannel`).
4. Click **Save**.
5. Go to the **Deployments** tab and **Redeploy** your latest production deployment so the new variables are loaded by the server.

#### For Local Development
1. Open the [`.env.local`](file:///c:/Users/User/Desktop/classapp/.env.local) file in your project.
